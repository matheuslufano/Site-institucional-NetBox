...
export function ClientShortcuts({ home = false }: { home?: boolean }) {
  ...
  return (
    <div className={`client-shortcuts${visible ? "" : " is-hidden"}`} aria-hidden={!visible}>
      <a className={itemClass("billing")} href={SECOND_COPY} target="_blank" rel="noreferrer" onClick={() => setSelected("billing")}>
      ...
      <a className={itemClass("services")} href={home ? "#servicos" : "/nossos-servicos"} onClick={() => setSelected("services")}>
      ...
      <a className={itemClass("coverage")} href="/#consulta" onClick={() => setSelected("coverage")}>
      ...
      <a className={itemClass("central")} href="/central-do-cliente" onClick={() => setSelected("central")}>
